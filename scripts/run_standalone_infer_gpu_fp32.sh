...
then
    ...
    echo "bash scripts/run_standalone_infer_gpu.sh [DATA_PATH] [CHECKPOINT]"
...
cp -r ../src ./infer_fp32
cd ./infer_fp32 || exit
echo "start infer for checkpoint file: ${CHECKPOINT_FILE_PATH}"
python infer.py --data_path=$PATH1 --checkpoint_file_path=$CHECKPOINT_FILE_PATH --device_target='GPU' > infer.log 2>&1 &
echo "end infer for checkpoint file: ${CHECKPOINT_FILE_PATH}"
cd ..